...
type Party struct {
	...
	goingToSettlement *geometry.Shape
...
func (p *Party) tryEnterSettlement() {
	...
	for _, s := range world.settlements {
		if p.goingToSettlement == s && p.hitbox.IsOverlappingShapes(s) {
...
func (party *Party) handlePlayer() {
	...
	for _, s := range settlements {
		var hovering = s.IsContainingPoint(mx, my)
		if hovering || s.IsContainingPoint(party.moveTargetX, party.moveTargetY) {
